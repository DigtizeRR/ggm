...
<div class="wrap ggm-health-admin">
	...
	<?php else : ?>
		...
		$builder_error_key = sanitize_key( wp_unslash( $_GET['error'] ?? '' ) );
		$builder_error = 'invalid_form' === $builder_error_key
			? __( 'The form could not be saved. Add a title and at least one field, section title, or section description.', 'ggm-member-dashboard' )
			: ( 'invalid_payment_options' === $builder_error_key ? __( 'Payment needs at least one price choice. Every choice needs a unique heading and an amount greater than zero.', 'ggm-member-dashboard' ) : '' );
		$schema = $form
		...
		$health_checkout_theme = ! empty( $settings['health_checkout_theme'] ) || ( $form && 3 === (int) $form->id && ! array_key_exists( 'health_checkout_theme', $settings ) );
		$payment_currencies = class_exists( 'GGM_Currency' ) ? GGM_Currency::currencies() : array( 'INR' => array( 'label' => 'Indian Rupee' ) );
		$saved_payment_currency = class_exists( 'GGM_Currency' )
			? GGM_Currency::normalize_code( $settings['payment_currency'] ?? ggm_get_setting( 'ggm_currency', 'INR' ), GGM_Currency::base_currency() )
			: 'INR';
		$payment_options = GGM_Form_Builder::get_payment_options( $settings );
		$payment_options_width = in_array( (string) ( $settings['payment_options_width'] ?? ( $payment_options[0]['width'] ?? '100' ) ), array( '30', '50', '100' ), true ) ? (string) ( $settings['payment_options_width'] ?? ( $payment_options[0]['width'] ?? '100' ) ) : '100';
			...
			<div class="ggm-builder-layout">
				...
				<div><strong><?php echo $uses_workshop_starter ? esc_html__( 'Workshop health form starter loaded', 'ggm-member-dashboard' ) : esc_html__( 'Blank form loaded', 'ggm-member-dashboard' ); ?></strong><p><?php echo $uses_workshop_starter ? esc_html__( 'This editable starter copies the 16 questions from the former workshop dashboard health intake. Member Dashboard placement is preselected; save it as Published when it is ready for members.', 'ggm-member-dashboard' ) : esc_html__( 'Build your own form from an empty section, or load the workshop health form starter.', 'ggm-member-dashboard' ); ?></p></div>
					...
					<section id="ggm-settings" class="ggm-builder-panel ggm-builder-tab-panel" role="tabpanel" aria-labelledby="ggm-builder-tab-settings" tabindex="0" hidden>
						...
						<div class="ggm-input-group ggm-span-2"><span class="ggm-input-label"><?php esc_html_e( 'Checkout Design', 'ggm-member-dashboard' ); ?></span><label class="ggm-checkbox-label"><input type="checkbox" name="health_checkout_theme" value="1" <?php checked( $health_checkout_theme ); ?>> <span><?php esc_html_e( 'Use the green health checkout design', 'ggm-member-dashboard' ); ?><small><?php esc_html_e( 'Applies only to this form. It gives its fields, plan selection, secure-payment cue, and payment button the supplied checkout design; all other forms keep their current design.', 'ggm-member-dashboard' ); ?></small></span></label></div>
						...
						<div class="ggm-input-group ggm-span-2 ggm-payment-addon-settings">
							<span class="ggm-input-label"><?php esc_html_e( 'Payment Button Add-on', 'ggm-member-dashboard' ); ?></span>
							<label class="ggm-checkbox-label"><input type="checkbox" name="payment_enabled" value="1" <?php checked( ! empty( $settings['payment_enabled'] ) ); ?>> <span><?php esc_html_e( 'Replace the final submit button with a paid button', 'ggm-member-dashboard' ); ?><small><?php esc_html_e( 'Visitors must choose one price and complete payment before their response is submitted.', 'ggm-member-dashboard' ); ?></small></span></label>
							<div class="ggm-inline-inputs"><label><?php esc_html_e( 'Button label', 'ggm-member-dashboard' ); ?><input type="text" name="payment_label" value="<?php echo esc_attr( $settings['payment_label'] ?? 'Pay & Submit' ); ?>" placeholder="<?php esc_attr_e( 'Pay & Submit', 'ggm-member-dashboard' ); ?>"></label><label><?php esc_html_e( 'Currency', 'ggm-member-dashboard' ); ?><select name="payment_currency"><?php foreach ( $payment_currencies as $currency_code => $currency_details ) : ?><option value="<?php echo esc_attr( $currency_code ); ?>" <?php selected( $saved_payment_currency, $currency_code ); ?>><?php echo esc_html( $currency_code . ' — ' . ( $currency_details['label'] ?? $currency_code ) ); ?></option><?php endforeach; ?></select></label></div>
							<div class="ggm-payment-options-editor" id="ggm-payment-options-editor">
								<div class="ggm-payment-options-editor__header"><strong><?php esc_html_e( 'Payment price choices', 'ggm-member-dashboard' ); ?></strong><small><?php esc_html_e( 'Add a heading and paid amount for every choice. Visitors can select only one.', 'ggm-member-dashboard' ); ?></small></div>
								<label class="ggm-payment-options-width"><?php esc_html_e( 'Price choices width', 'ggm-member-dashboard' ); ?><select name="payment_options_width"><?php foreach ( array( '100' => __( '100%', 'ggm-member-dashboard' ), '50' => __( '50%', 'ggm-member-dashboard' ), '30' => __( '30%', 'ggm-member-dashboard' ) ) as $width => $width_label ) : ?><option value="<?php echo esc_attr( $width ); ?>" <?php selected( $payment_options_width, $width ); ?>><?php echo esc_html( $width_label ); ?></option><?php endforeach; ?></select><small><?php esc_html_e( 'Sets the width of the entire price-selection list. Individual prices stay in one column.', 'ggm-member-dashboard' ); ?></small></label>
								<div id="ggm-payment-options-list">
									<?php foreach ( $payment_options as $price_index => $price_option ) : ?>
										<div class="ggm-payment-option-row">
											<input type="hidden" name="payment_options[<?php echo esc_attr( $price_index ); ?>][id]" value="<?php echo esc_attr( $price_option['id'] ); ?>">
											<label><?php esc_html_e( 'Price heading', 'ggm-member-dashboard' ); ?><input type="text" name="payment_options[<?php echo esc_attr( $price_index ); ?>][label]" value="<?php echo esc_attr( $price_option['label'] ); ?>" placeholder="<?php esc_attr_e( 'e.g. Standard Registration', 'ggm-member-dashboard' ); ?>"></label>
											<label><?php esc_html_e( 'Plan subtitle', 'ggm-member-dashboard' ); ?><input type="text" name="payment_options[<?php echo esc_attr( $price_index ); ?>][description]" value="<?php echo esc_attr( $price_option['description'] ?? '' ); ?>" placeholder="<?php esc_attr_e( 'e.g. Basic health guidance', 'ggm-member-dashboard' ); ?>"></label>
											<label><?php esc_html_e( 'Amount', 'ggm-member-dashboard' ); ?><input type="number" min="0.01" step="0.01" name="payment_options[<?php echo esc_attr( $price_index ); ?>][amount]" value="<?php echo esc_attr( $price_option['amount'] ); ?>" placeholder="499"></label>
											<button type="button" class="button-link-delete ggm-remove-payment-option"><?php esc_html_e( 'Remove', 'ggm-member-dashboard' ); ?></button>
										</div>
									<?php endforeach; ?>
								</div>
								<button type="button" class="button" id="ggm-add-payment-option"><?php esc_html_e( 'Add price choice', 'ggm-member-dashboard' ); ?></button>
							</div>
							<label><?php esc_html_e( 'Payment description', 'ggm-member-dashboard' ); ?><input type="text" name="payment_description" value="<?php echo esc_attr( $settings['payment_description'] ?? '' ); ?>" placeholder="<?php esc_attr_e( 'Shown in Razorpay checkout', 'ggm-member-dashboard' ); ?>"></label>
							<div class="ggm-rich-message-editor"><label for="form_payment_success"><?php esc_html_e( 'Paid success message', 'ggm-member-dashboard' ); ?></label><?php wp_editor( $settings['payment_success'] ?? 'Payment received. Your response has been submitted.', 'form_payment_success', array_merge( $rich_editor_settings, array( 'textarea_name' => 'payment_success', 'textarea_rows' => 5 ) ) ); ?></div>
						</div>
						<script>
						(function(){
							var list=document.getElementById('ggm-payment-options-list'),add=document.getElementById('ggm-add-payment-option');
							if(!list||!add)return;
							add.addEventListener('click',function(){
								var index='new_'+Date.now()+'_'+Math.floor(Math.random()*1000),row=document.createElement('div');
								row.className='ggm-payment-option-row';
								row.innerHTML='<input type="hidden" name="payment_options['+index+'][id]" value=""><label><?php echo esc_js( __( 'Price heading', 'ggm-member-dashboard' ) ); ?><input type="text" name="payment_options['+index+'][label]" placeholder="<?php echo esc_js( __( 'e.g. Standard Registration', 'ggm-member-dashboard' ) ); ?>"></label><label><?php echo esc_js( __( 'Plan subtitle', 'ggm-member-dashboard' ) ); ?><input type="text" name="payment_options['+index+'][description]" placeholder="<?php echo esc_js( __( 'e.g. Basic health guidance', 'ggm-member-dashboard' ) ); ?>"></label><label><?php echo esc_js( __( 'Amount', 'ggm-member-dashboard' ) ); ?><input type="number" min="0.01" step="0.01" name="payment_options['+index+'][amount]" placeholder="499"></label><button type="button" class="button-link-delete ggm-remove-payment-option"><?php echo esc_js( __( 'Remove', 'ggm-member-dashboard' ) ); ?></button>';
								list.appendChild(row);row.querySelector('input[type=text]').focus();
							});
							list.addEventListener('click',function(event){var button=event.target.closest('.ggm-remove-payment-option');if(button){event.preventDefault();button.closest('.ggm-payment-option-row').remove();}});
						})();
						</script>
					</section>
...
.ggm-richtext{border:1px solid #dce1ea;border-radius:7px;background:#fff;overflow:hidden}.ggm-richtext-toolbar{display:flex;gap:2px;padding:4px;background:#f8f9fc}.ggm-richtext>.ggm-richtext-toolbar{border-bottom:1px solid #e4e8f0}.ggm-richtext-toolbar button{min-width:26px;height:24px;padding:0 6px;border:1px solid transparent;border-radius:5px;background:transparent;color:#4d5768;cursor:pointer;font-size:12px;white-space:nowrap}.ggm-richtext-toolbar button:hover,.ggm-richtext-toolbar button:focus-visible{background:#eef2ff;border-color:#c7d2fe}.ggm-richtext-input{min-height:70px;padding:10px 11px;outline:none}.ggm-richtext-input:empty:before{content:attr(aria-label);color:#98a2b3}.ggm-richtext--inline{display:flex;align-items:center;gap:4px;border:0;background:transparent}.ggm-richtext--inline .ggm-richtext-toolbar{border:0;background:transparent;padding:0;flex:0 0 auto}.ggm-richtext-input--inline{flex:1;min-width:0;min-height:36px;padding:8px 11px;border:1px solid #dce1ea;border-radius:7px;white-space:pre-wrap;overflow-wrap:anywhere}
		...
		.ggm-payment-options-editor{display:flex;flex-direction:column;gap:10px;padding:14px;border:1px solid #dfe3eb;border-radius:9px;background:#fafbfc}.ggm-payment-options-editor__header strong,.ggm-payment-options-editor__header small,.ggm-payment-options-width small{display:block}.ggm-payment-options-editor__header small,.ggm-payment-options-width small{margin-top:3px;color:#697386}.ggm-payment-options-width{display:flex;flex-direction:column;gap:5px;width:min(280px,100%);font-weight:600}.ggm-payment-option-row{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) 150px auto;gap:10px;align-items:end;margin:8px 0}.ggm-payment-option-row label{display:flex;flex-direction:column;gap:5px}.ggm-payment-option-row .button-link-delete{padding-bottom:10px;color:#b42318}@media(max-width:600px){.ggm-payment-option-row{grid-template-columns:1fr}.ggm-payment-option-row .button-link-delete{justify-self:start;padding-bottom:0}}
		...
		function flagSvg(iso){iso=String(iso||'IN').toUpperCase().replace(/[^A-Z]/g,'');var country=countryCodes.find(function(item){return String(item.iso||'').toUpperCase()===iso});return country&&country.flag_svg?country.flag_svg:''}
		...
		function countrySelectCfg(key,label,value){var selected=countryCodes.find(function(c){return c.iso===String(value||'').toUpperCase()})||countryCodes.find(function(c){return c.iso==='IN'})||countryCodes[0],h='<label class="ggm-config-control ggm-country-config"><span>'+label+'</span><span class="ggm-country-config-preview">'+flagSvg(selected.iso)+' '+esc(selected.dial)+'</span><select data-config="'+key+'">';countryCodes.forEach(function(c){h+='<option value="'+esc(c.iso)+'" data-dial="'+esc(c.dial)+'" '+(String(value)===String(c.iso)?'selected':'')+'>'+esc(c.dial)+'</option>'});return h+'</select></label>'}
